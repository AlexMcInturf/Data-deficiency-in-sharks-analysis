...
```{r}
ic <- data.frame(Model = c("secondorder.mod", "secondorder.mod2", "secondorder.mod3", "secondorder.mod4", "firstorder.mod", "firstorder.mod2"),
AIC = c(AIC(secondorder.mod), AIC(secondorder.mod4), AIC(secondorder.mod2), AIC(secondorder.mod3), AIC(firstorder.mod), AIC(firstorder.mod2)),
...
summary(secondorder.mod3) # no abyssopelagic, first order transoceanic

```
# AIC selection  
*Generally seeing that second order models do functionally equally (even including abyssopelagic), except the one where we included transoceanic x ocean basin parameters*  
