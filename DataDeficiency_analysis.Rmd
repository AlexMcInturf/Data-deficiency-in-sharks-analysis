...
firstorder.mod <- glm(Data.Deficient ~ Deepwater + Coastal + Pelagic + Benthic + BrackishFreshwater +
      ...
      Fisheries + commercial + subsistence + Gamefish + Aquarium + Human.Uses+ D1 + D2 + D3 +
...
### Potential second order model
secondorder.mod <- glm(Data.Deficient ~ Deepwater + Coastal + Pelagic + Benthic + BrackishFreshwater +
      Tropical + Temperate + 
      Global + Pacific + Atlantic + Indian + Arctic + Antarctic + Trans.oceanic +
      Fisheries + commercial + subsistence + Gamefish + Aquarium + Human.Uses+ D1 + D2 + D3 +
      log_size + Rep_Strategy + Coastal*Fisheries + Pelagic*Fisheries + Benthic*Fisheries + Trans.oceanic*Fisheries + Global*Fisheries, data = dd, family = "binomial")
...
## extract all coefficients, make into a data frame
...
########### Evan's Help ####
library(emmeans) #computing estimated marginal means (least square means)
#Least square means are means for groups that are adjusted for means of other factors in the model (allows us to account for different sample sizes between different families, etc.)
# can add conditions that say waht are the mean probability for some given variables possibly given another or some variable beforehand


logsize_means <- emmeans(firstorder.mod, ~ log_size, 
         var = "log_size", 
         at = list(log_size = seq(2, 8, by = .25)),
         type = "response") #look into var or spec, include statment on estimated mean of DD that is fisheries|pelagic, etc.
...
data.frame(logsize_means) %>%
  ...
  geom_line() +
  theme_bw() +
  xlab("Average species size (cm)") +
  ylab("Probability of Data Deficiency")
#  



### Exploring potential second-order interactions
trash1 <- emmeans(firstorder.mod, ~ Coastal|Fisheries, 
         var = "Fisheries|Coastal", 
         at = list(Coastal = seq(0, 1, by = 1)),
         type = "response") 

trash1.2 <- emmeans(firstorder.mod, ~ Coastal, 
         var = "Coastal", 
         at = list(Coastal = seq(0, 1, by = 1)),
         type = "response")
...
trash2 <- emmeans(firstorder.mod, ~ Pelagic|Fisheries, 
         var = "Fisheries|Pelagic", 
         at = list(Pelagic = seq(0, 1, by = 1)),
         type = "response") 
trash2

trash2.2 <- emmeans(firstorder.mod, ~ Pelagic, 
         var = "Pelagic", 
         at = list(Pelagic = seq(0, 1, by = 1)),
         type = "response")

data.frame(trash1.2) %>%
  ggplot(aes(x = Coastal, 
             y = prob)) +
  geom_ribbon(aes(ymin = asymp.LCL,
              ymax = asymp.UCL),
              alpha = .2) +
...
data.frame(trash1) %>%
  ...
  geom_line() +
  theme_bw() +
  xlab("Coastal|Fisheries") +
  ylab("Probability of Data Deficiency")

data.frame(trash2) %>%
  ggplot(aes(x = Pelagic, 
             y = prob)) +
  geom_ribbon(aes(ymin = asymp.LCL,
              ymax = asymp.UCL),
              alpha = .2) +
